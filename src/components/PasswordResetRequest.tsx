...
import { ArrowLeft, Mail, Pill, Send } from 'lucide-react';
...
export default function PasswordResetRequest() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(status === 429
        ? 'Muitas solicitações. Aguarde pelo menos um minuto antes de tentar novamente.'
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-5">
              ...
              <label className="block space-y-2">
                ...
                <div className="relative">
                  ...
                  <input type="email" required autoFocus maxLength={254} autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} className="min-h-12 w-full rounded-lg border border-slate-200 py-3 pl-11 pr-3 font-semibold text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
                </div>
              </label>
